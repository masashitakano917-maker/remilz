import"./supabase-CGTu0p87.js";/* empty css              */import{i}from"./auth-Dtda0RQ8.js";import{h as o}from"./data-5T_OKylw.js";import"https://esm.sh/@supabase/supabase-js@2";const s=document.body.dataset.current;document.querySelectorAll("[data-nav]").forEach(t=>{t.dataset.nav===s&&t.classList.add("active")});i().then(t=>{const e=document.getElementById("apply-panel");t&&e&&(e.innerHTML='<h3>この求人に応募する</h3><p>応募ボタンを押すと企業にプロフィールが送信されます。</p><button class="btn btn-primary" id="apply-btn">この求人に応募する</button><p id="apply-msg" style="display:none;margin-top:8px;color:var(--green);font-weight:700;font-size:13px">応募しました！企業からの連絡をお待ちください。</p>',document.getElementById("apply-btn").addEventListener("click",()=>{document.getElementById("apply-btn").disabled=!0,document.getElementById("apply-btn").textContent="応募済み",document.getElementById("apply-msg").style.display=""}))});const r=new URLSearchParams(window.location.search),a=r.get("id");a&&(async()=>{try{const t=await o(a);if(!t)return;document.querySelector(".page-title").textContent=t.title,document.querySelector(".page-lead").textContent=t.description||"",document.querySelector(".hero-side-card p").textContent=(t.tags||[]).join(" / ");const e=document.querySelector(".detail-main");e.innerHTML=`
          <h2>募集概要</h2>
          <p>${t.description||""}</p>
